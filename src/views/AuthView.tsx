...
export const AuthView = () => {
    ...
    return (
        <div
            className={`relative w-full min-h-screen overflow-hidden shadow-lg bg-(--bg-1) ${isLogin ? "login" : "register"
                }`}
        >
            {/* FORM SECTION */}
            <div
                className={`absolute w-full md:w-1/2 h-full justify-center items-center flex flex-col top-0 z-20 transition-transform duration-1500 ease-in-out bg-(--bg-1) text-(--text-1) px-4 md:px-0 ${isLogin ? "translate-x-0" : "md:translate-x-full translate-x-0"
                    }`}
